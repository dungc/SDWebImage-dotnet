...
public enum SDWebImageOptions : ulong {
    ...
    /// </summary>
    DelayPlaceholder = 1 << 8,
    
    /// <summary>
    /// We usually don't call transformDownloadedImage delegate method on animated images.
    /// </summary>
    TransformAnimatedImage = 1 << 9,
    
    /// <summary>
    /// By default, image is added to the imageView after download.
    /// </summary>
    AvoidAutoSetImage = 1 << 10,
    
    /// <summary>
    /// By default, images are decoded respecting their original size.
    /// </summary>
    ScaleDownLargeImages = 1 << 11,
    
    /// <summary>
    /// By default, we decode the animated image. This flag can force decode the first frame only.
    /// </summary>
    DecodeFirstFrameOnly = 1 << 12,
    
    /// <summary>
    /// By default, for SDAnimatedImage, we decode the animated image frame during rendering to reduce memory usage.
    /// </summary>
    PreloadAllFrames = 1 << 13,
    
    /// <summary>
    /// By default, when you use SDWebImageContextAnimatedImageClass, we may still use UIImage when the memory cache hit.
    /// </summary>
    MatchAnimatedImageClass = 1 << 14,
    
    /// <summary>
    /// By default, when the image is loading, the imageView will not change.
    /// </summary>
    WaitStoreCache = 1 << 15,
    ...
    AvoidDecodeImage = 1 << 16,
    
    /// <summary>
    /// By default, when you use SDWebImageContextAnimatedImageClass, we may still fallback to normal UIImage.
    /// </summary>
    FromLoaderOnly = 1 << 17,
    
    /// <summary>
    /// By default, when you use SDWebImageTransition, the transition will only applies for download image from network.
    /// </summary>
    ForceTransition = 1 << 18,
    
    /// <summary>
    /// By default, we will query the memory cache, disk cache, then network. This flag can force to query network only.
    /// </summary>
    FromCacheOnly = 1 << 19
}

// SDWebImageDownloaderOptions 枚举定义 - 同样改为 ulong
[Native]
public enum SDWebImageDownloaderOptions : ulong {
    ...
    /// By default, we decode the animated image. This flag can force decode the first frame only.
    /// </summary>
    DecodeFirstFrameOnly = 1 << 9,
    
    /// <summary>
    /// By default, for SDAnimatedImage, we decode the animated image frame during rendering.
    /// </summary>
    PreloadAllFrames = 1 << 10,
    
    /// <summary>
    /// By default, we will decode the image in the background during cache query and download.
    /// </summary>
    [Obsolete("Use SDWebImageContextImageForceDecodePolicy instead")]
    AvoidDecodeImage = 1 << 11,
    
    /// <summary>
    /// By default, when you use SDWebImageContextAnimatedImageClass, we may still use UIImage.
    /// </summary>
    MatchAnimatedImageClass = 1 << 12
}
